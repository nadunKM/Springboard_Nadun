...
/* The CONCAT didn't work for me so I couldn't join names */
...
SELECT DISTINCT firstname, surname, Facilities.name, membercost
FROM Members
INNER JOIN Bookings
ON Members.memid = Bookings.memid
INNER JOIN Facilities
...
SELECT SUM(membercost) + SUM(guestcost) AS revenue
FROM Facilities
...
(SELECT name FROM Facilities) AS facilityname
FROM Members
